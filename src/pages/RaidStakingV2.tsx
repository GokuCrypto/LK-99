...
const RaidStakingV2 = () => {
  ...
  const approveRAIDTo = async (to = Constants.Contract.DONATEPLEDEGV2) => {
  ...
  const isRAIDApprovedTo = async (address: string, to = Constants.Contract.DONATEPLEDEGV2) => {
  ...
  const idobuy = async () => {
    ...
    const isRaidApprovedToDonate = await isRAIDApprovedTo(account, Constants.Contract.DONATEPLEDEGV2)
    console.log('isRaidApprovedToDonate', isRaidApprovedToDonate)
    if (!isRaidApprovedToDonate) await approveRAIDTo(Constants.Contract.DONATEPLEDEGV2)
    toastSuccess(
      `${t('Token Staking')}!`,
  ...
  const donate = async () => {
    const contract = await getContractHandler('DONATEPLEDEGV2')
    if (!contract) return
  ...
  /* 查询剩余额度 */
  const limit = async () => {
    const contractss = await getContractHandler('DONATEPLEDEGV2')
  ...
  const getLastBlock = async () => {
    ...
    console.log('blockblockblockblock2', BigNumber.from(res).toNumber())
    ...
    if (BigNumber.from(res).toNumber() > BigNumber.from(block).toNumber()) {
      ...
      setLastTime(pool == 0 ? '15 days later' : '30 days later')
    }
    // eslint-disable-next-line consistent-return
    return res
  }
  ...
  if (account != null) {
    isRAIDApprovedTo(account, Constants.Contract.DONATEPLEDEGV2)
  }
      ...
      <div className="mas">
        ...
        <div className="mas_head2">
          ...
          <div>
            ...
            <h1 className="mas_head2_h2">{rewardTime * (pool + 1)} DAY</h1>
          </div>
        </div>
        ...
        <div className="mas_head4">
          ...
          <div className="mas_head4_1">
            ...
            <div className="mas_head4_1_right">{pool == 0 ? 20000000 : 10000000} </div>
          </div>

          <div className="mas_head4_1">
            <div className="mas_head4_1_left"> {t('Rate')} </div>
            <div className="mas_head4_1_right">{pool == 0 ? '1%' : '3%'}</div>
          </div>
