...
const RaidStaking = () => {
  ...
  const approveRAIDTo = async (to = Constants.Contract.DONATEPLEDEG) => {
  ...
  const isRAIDApprovedTo = async (address: string, to = Constants.Contract.DONATEPLEDEG) => {
  ...
  const idobuy = async () => {
    ...
    const isRaidApprovedToDonate = await isRAIDApprovedTo(account, Constants.Contract.DONATEPLEDEG)
    console.log('isRaidApprovedToDonate', isRaidApprovedToDonate)
    if (!isRaidApprovedToDonate) await approveRAIDTo(Constants.Contract.DONATEPLEDEG)
    toastSuccess(
      `${t('Token Staking')}!`,
  ...
  const donate = async () => {
    const contract = await getContractHandler('DONATEPLEDEG')
    if (!contract) return
  ...
  /* 查询剩余额度 */
  const limit = async () => {
    const contractss = await getContractHandler('DONATEPLEDEG')
  ...
  if (account != null) {
    isRAIDApprovedTo(account, Constants.Contract.DONATEPLEDEG)
  }
      ...
      <div className="mas">
        ...
        <div className="mas_head4">
          ...
          <div className="mas_head4_1">
            ...
            <div className="mas_head4_1_right">{pool == 0 ? '5%' : '10%'}</div>
          </div>
